...
@SpringBootTest @RunWith(SpringRunner.class) public class SyncBlocksTest implements WalletConst {
  
    private final String TAG = TAG_TEST + "[syncBlocks]";
    @Autowired SharedTest testCase;
    
    @Test
    public void syncBlocks() throws Exception {
        String[] ENABLED_SYMBOLS = {SYMBOL_KAI};
        int success = 0;
        for (String symbol : ENABLED_SYMBOLS) {
            success += (testCase.syncBlocks(symbol)==true)?1:0;
        }
