...
@SpringBootTest @RunWith(SpringRunner.class) public class SendFromRequestTest implements WalletConst {
  
    private final String TAG = TAG_TEST + "[sendRequest]";
    @Autowired SharedTest testCase;
    
    @Test
    public void sendFromRequest() throws Exception {
        String[] ENABLED_SYMBOLS = {SYMBOL_KAI};
        int success = 0;
        for (String symbol : ENABLED_SYMBOLS) {
//            success += (testCase.sendFromRequest(symbol)==true)?1:0;
        }
