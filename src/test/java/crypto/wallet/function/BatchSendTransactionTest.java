...
@SpringBootTest @RunWith(SpringRunner.class) public class BatchSendTransactionTest implements WalletConst {
  
    private final String TAG = TAG_TEST + "[sendRequest]";
    @Autowired SharedTest testCase;
    
    @Test
    public void batchSendTransaction() throws Exception {
        String[] ENABLED_SYMBOLS = {SYMBOL_KAI};
        int success = 0;
        for (String symbol : ENABLED_SYMBOLS) {
            success += (testCase.batchSendTransaction(symbol)==true)?1:0;
        }
